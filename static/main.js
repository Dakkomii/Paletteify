// Displaying the user input field function for editProfile.html 
function displayUsernameInput(e) {
    e.preventDefault();
    var usernameInput = document.getElementById("editUsernameInput");
    if (usernameInput.display == "block") {
        usernameInput.style.display = "none";
    }
    else {
        usernameInput.style.display = "block";
        document.getElementById("editUsernameButton").style.display = "none";
        document.getElementById("saveUsernameButton").style.display = "block";
    }
}

const img = document.getElementById("profileImage")
img.addEventListener("error", function(event) {
  event.target.src = "data:image/png;base64,"
  event.onerror = null
})

//Makes sure that the type of content cheboxes only accept one answer
var checks = document.querySelectorAll(".check");
var max = 1;
for (var i = 0; i < checks.length; i++)
  checks[i].onclick = selectiveCheck;
function selectiveCheck (event) {
  var checkedChecks = document.querySelectorAll(".check:checked");
  if (checkedChecks.length > max)
    return false;
}

//converts metadata to lowercase
function convert() {
    var str ;
    str = document.getElementById("metadata");
    str.value = (str.value.toLowerCase());
    var form = document.getElementById("form1");
    form.submit();
}

//Makes sure that the type of content checkboxes only accept one answer
var checks = document.querySelectorAll(".search");
var max = 1;
for (var i = 0; i < checks.length; i++)
  checks[i].onclick = selectiveCheck;
function selectiveCheck (event) {
  var checkedChecks = document.querySelectorAll(".search:checked");
  if (checkedChecks.length > max)
    return false;
}
